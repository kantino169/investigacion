  ...
  <div class="row justify-center">
    <q-stepper ref="stepper" color="primary" v-model="step" :alternative-labels="alt" vertical>
      ...
      <q-step name="parte2" title="II. DESCRIPCION DEL PROYECTO">
        <p>linea, grupo y ese lio</p>
        <q-input float-label="Titulo del proyecto"></q-input>
        <q-input type="textarea" float-label="4. Resumen tecnico - (entre 150-250 palabras)"></q-input>
        <q-input type="textarea" float-label="5. Palabras clave - (entre 4 y 6 descriptores tematicos relevantes)"></q-input>
        <q-stepper-navigation v-if="!globalNavigation">
      ...
      <q-step name="parte3" title="III. INTRODUCCION DEL PROYECTO">
        <q-input type="textarea" float-label="1. Estado actual del conocimiento sobre el tema (Marco teorico que incluya la descripcion de los principales datos de la literatura)"></q-input>
        <q-input type="textarea" float-label="2. Definicion del problema e hipotesis (dependiendo del tipo de estudio)"></q-input>
        <q-input type="textarea" float-label="3. Justificacion cientifica, academica-institucional y social"></q-input>
        <q-input type="textarea" float-label="4. Objetivos"></q-input>
        <q-stepper-navigation v-if="!globalNavigation">
          <q-btn color="primary" @click="$refs.stepper.next()">Continue</q-btn>
          <q-btn class="q-ml-sm" color="secundary" flat @click="$refs.stepper.previous()">Atras</q-btn>
        </q-stepper-navigation>
      ...
      <q-step name="parte4" title="IV. METODOS">
        <q-input type="textarea" float-label="1. Metodologia (conforme el tipo de investigacion)"></q-input>
        ...
        <q-input type="textarea" float-label="2. Anexos (conforme el tipo de investigacion)"></q-input>
        <q-collapsible icon="help_outline" label="Mas informacion">
          <div>
      ...
      <q-step name="parte5" title="V. PUBLICACION CIENTIFICA">
        <q-input type="textarea" float-label="1. Presentar un plan viable de publicación de la investigación (Especificar si se pretende publicar los resultados de la investigación en forma de un artículo científico o un libro)."></q-input>
        <q-stepper-navigation v-if="!globalNavigation">
          <q-btn color="primary" @click="$refs.stepper.next()">Continue</q-btn>
          <q-btn class="q-ml-sm" color="secundary" flat @click="$refs.stepper.previous()">Atras</q-btn>
        </q-stepper-navigation>
      ...
      <q-step name="parte6" title="VI. PRESUPUESTO">
        <q-input type="number" float-label="1. Estimacion de la duracion del proyecto (expresado en cantidad de años)"></q-input>
        <q-input type="number" float-label="2. Estimacion del tiempo semanal necesario para realizar la investigacion (expresado en cantidad de horas reloj semanal por miembro de equipo)"></q-input>
        <q-stepper-navigation v-if="!globalNavigation">
          <q-btn color="primary" @click="$refs.stepper.next()">Continue</q-btn>
          <q-btn class="q-ml-sm" color="secundary" flat @click="$refs.stepper.previous()">Atras</q-btn>
        </q-stepper-navigation>
      ...
      <q-step name="parte7" title="VII. REFERENCIAS BIBLIOGRAFICAS">
        <q-input
          type="textarea"
          float-label="1.Listado de referencias bibliograficas (citados en el proyecto)" />
        <q-stepper-navigation v-if="!globalNavigation">
          <q-btn class="q-ml-sm" color="primary">Finalizar</q-btn>
...
export default {
  ...
  methods: {
    ...mapActions('proyecto', ['cargarModalidades'])
  }
}
</script>
